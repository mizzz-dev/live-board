...
export class CanvasLayerCache {
  ...
  getRasterLayer(
    ...
    if (cached?.signature === signature) return { canvas: cached.canvas, hit: true };
...
function renderRasterDrawing(
  ...
  for (const stroke of layer.content.drawing.strokes) {
    renderStroke(context, stroke);
  }
  for (const fill of layer.content.drawing.fills) {
...
function renderStroke(context: CanvasContext, stroke: RasterStroke): void {
  ...
  for (let index = 0; index < points.length; index += 1) {
    ...
    const taper = taperFactor(fraction, stroke.taperStart, stroke.taperEnd);
    const pressure = pressureFactor(point.pressure);
    const radius =
      (stroke.size / 2) * taper * (stroke.pressureSize ? pressure : 1);
    const opacity = stroke.opacity * (stroke.pressureOpacity ? pressure : 1);
    stamp(context, point, radius, stroke.color, opacity, stroke.hardness);
    if (next !== undefined) {
      stampSegment(
        context,
        point,
...
function stampSegment(
  ...
  for (let index = 1; index <= count; index += 1) {
    ...
    const pressure = pressureFactor(lerp(from.pressure, to.pressure, ratio));
    const radius = lerp(
      fromRadius,
    ...
    stamp(
      context,
      { x: lerp(from.x, to.x, ratio), y: lerp(from.y, to.y, ratio) },
      radius,
      stroke.color,
      opacity,
      stroke.hardness,
...
function renderVectorLayer(
  ...
  switch (layer.type) {
    ...
    case 'shape':
      ...
      if (layer.content.fill !== null) context.fillStyle = layer.content.fill;
    ...
    case 'image':
      ...
      context.strokeRect(0, 0, layer.content.width, layer.content.height);
      context.restore();
      return;
  }
}
...
function taperFactor(fraction: number, start: number, end: number): number {
