...
test('Browser PreviewではMain Process依存の保存操作を無効化する', async ({
  ...
  await expect(
    page.getByRole('button', { name: '名前を付けて保存' }),
  ).toBeDisabled();
  await expect(page.getByRole('button', { name: '開く', exact: true })).toBeDisabled();
  await expect(page.getByRole('button', { name: 'インポート' })).toBeDisabled();
