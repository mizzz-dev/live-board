  ...
  function transformSelection(
    ...
    if (
      ...
      setSelection((current) =>
        current === null ? null : moveSelection(current, dx, dy),
      );
    ...
    if (action === 'grow' || action === 'shrink') {
      ...
      setSelection((current) =>
        current === null ? null : scaleSelection(current, scale, scale),
      );
      updateTransform({
        ...transform,
    ...
    setSelection((current) =>
      current === null ? null : rotateSelection(current, rotation),
    );
      ...
      <div className="selection-actions" aria-label="選択対象の基本変形">
        {([
          ...
          <button
            ...
            onClick={() => transformSelection(action)}
          >
            {label}
          </button>
        ))}
    ...
    <div className="content-editor">
      ...
      <label>
        ...
        <input
          ...
          onChange={(event) => onChange({ ...content, fontFamily: event.currentTarget.value })}
        />
      </label>
      ...
      <label>
        ...
        <select
          ...
          onChange={(event) => onChange({ ...content, fontWeight: Number(event.currentTarget.value) })}
        >
      ...
      <label>
        <input
          type="checkbox"
          checked={content.fontStyle === 'italic'}
          onChange={(event) => onChange({ ...content, fontStyle: event.currentTarget.checked ? 'italic' : 'normal' })}
        />
        斜体
      </label>
      <label>
        ...
        <select
          value={content.align}
          onChange={(event) => onChange({ ...content, align: event.currentTarget.value as RichTextContent['align'] })}
        >
...
function ImageEditor({
  ...
  layer: ImageLayer;
  onChange(content: RichImageContent): void;
}) {
  const content = getRichImageContent(layer);
  const updateCrop = (patch: Partial<RichImageContent['crop']>) =>
    onChange({ ...content, crop: { ...content.crop, ...patch } });
  return (
    <div className="content-editor">
...
function ShapeEditor({
  ...
  layer: ShapeLayer;
  onChange(content: RichShapeContent): void;
}) {
  ...
  return (
    <div className="content-editor">
      <label>
        ...
        <select
          value={content.shape}
          onChange={(event) => onChange({ ...content, shape: event.currentTarget.value as RichShapeContent['shape'] })}
        >
