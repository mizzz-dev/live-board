    ...
    try {
      ...
      for (const [index, file] of files.entries()) {
        if (file.size < 1 || file.size > MAX_ASSET_BYTES) {
          throw new Error(
            `${file.name || source}は25MB以内の画像を選択してください`,
          );
        }
        inputs.push({
      ...
      setLocalError(
        caught instanceof Error ? caught.message : '画像の読み込みに失敗しました',
      );
    } finally {
      setReading(false);
    ...
    <section className="asset-panel">
      ...
      >
        <input
          ...
          onChange={(event) =>
            void readAndImport([...(event.currentTarget.files ?? [])], 'file')
          }
        />
        <button
          type="button"
      ...
      {visibleError === null ? null : (
        <p className="asset-error" role="alert">{visibleError}</p>
      )}
        ...
        {library.assets.map((asset) => (
          <article key={asset.id} className="asset-row">
            ...
            <div>
              ...
              {asset.fileNames.length > 1 ? (
                <small>別名: {asset.fileNames.slice(1).join('・')}</small>
              ) : null}
              <small>
                {asset.width} × {asset.height} / {formatBytes(asset.byteLength)}
              </small>
              <small>
                {asset.sanitized ? 'SVGサニタイズ済み' : asset.mime}
              </small>
            </div>
          </article>
        ))}
      ...
      <p className="asset-total">
        保存バイナリ: {formatBytes(library.totalBytes)}
      </p>
    </section>
  );
}
